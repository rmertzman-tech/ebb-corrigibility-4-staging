...
def nav_clean(page):
    page.evaluate('localStorage.clear()'); page.evaluate("localStorage.setItem('navigator_ai_processing_consent_v1','accepted-v1')"); page.reload(wait_until='domcontentloaded'); page.evaluate("showWorkspace('twin')"); page.wait_for_timeout(80)

def ex_clean(page):
    page.evaluate('localStorage.clear()'); page.evaluate("localStorage.setItem('examined_disclosure_v1','acknowledged')"); page.reload(wait_until='domcontentloaded'); page.evaluate("()=>{state.workspace=true;document.getElementById('landing').style.display='none';document.getElementById('workspace').style.display='block';swTab('companion');}"); page.wait_for_timeout(80)
...
def last_assistant_nav(page):
    chat=page.evaluate("()=>JSON.parse(localStorage.getItem('navigator_twin_v2_chat')||'[]')")
    a=[x for x in chat if x.get('role')=='assistant']; return (a[-1] if a else None),chat

def last_assistant_ex(page):
    st=page.evaluate("()=>state.chatHistory")
    a=[x for x in st if x.get('role')=='assistant']; return (a[-1] if a else None),st
...
try:
    ...
    with sync_playwright() as p:
        ...
        nav.goto(NAV_URL,wait_until='domcontentloaded',timeout=args.timeout_ms); ex.goto(EX_URL,wait_until='domcontentloaded',timeout=args.timeout_ms)
        nav_build=nav.locator('meta[name="navigator-build"]').get_attribute('content'); ex_build=ex.locator('meta[name="examined-build"]').get_attribute('content')
        ...
        for idx,(case,tutor) in enumerate(expanded,1):
            cid=case['id']; rec={'index':idx,'case_id':cid,'regime':case['regime'],'tutor':tutor,'user_message':case['user_message'],'required_behavior':case['required_behavior'],'failure_target':case['failure_target'],'response_text':'','status':'','error':'','backend_route':'live' if not args.mock else 'mock'}
            try:
                ...
                if tutor=='Navigator Twin':
                    ...
                    nav.locator('#chat-input').fill(case['user_message']); nav.evaluate('()=>sendToTwin()')
                    a,chat=last_assistant_nav(nav)
                    ...
                    else: rec['status']='ok' if rec['response_text'] else 'error'
                else:
                    ex_clean(ex); setup_ex(ex,case)
                    before_calls=len(provider_requests)
                    ex.locator('#chat-input').fill(case['user_message']); ex.evaluate('()=>sendMessage()')
                    a,chat=last_assistant_ex(ex)
                    rec['candidate_build']=ex_build
                    rec['response_text']=(a or {}).get('content','')
                    rec['response_provenance']=(a or {}).get('provenance','')
                    rec['provider_request_delta']=len(provider_requests)-before_calls if not args.mock else None
                    rec['status']='ok' if rec['response_text'] else 'error'
                if not rec['response_text'] and rec['status']!='ok': rec['error']='no assistant response persisted'
            ...
            results.append(rec); print(f"{idx:02d}/29 {cid} {tutor}: {rec['status']}")
        browser.close()
finally:
    server.terminate()
...
summary={'build':'CORRIGIBILITY-4','mode':'MOCK_DRY_RUN' if args.mock else 'LIVE_SEMANTIC','records':len(results),'ok':sum(r['status']=='ok' for r in results),'errors':sum(r['status']!='ok' for r in results),'provider_requests_seen':len(provider_requests),'candidate_hashes':{},'results':results,'infrastructure_errors':infrastructure_errors}
for label,path in [('examined',ROOT/'Examined-main'/'index.html'),('navigator',ROOT/'SPC-FALL-2026-Navigator-main'/'index.html')]: summary['candidate_hashes'][label+'_index_sha256']=hashlib.sha256(path.read_bytes()).hexdigest()
